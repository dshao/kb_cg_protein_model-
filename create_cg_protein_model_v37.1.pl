...
#  system "convpdb.pl -renumber 1 -nohetero -out charmm27 temp.pdb > $out";   
  ...
  while(<IN>)
    ...
    if($_ =~ m/ATOM/) # Only read in lines that start with 'ATOM'
      ...
      print "dat_0: $dat[0]\n";
      print "dat_1: $dat[1]\n";
      print "dat_2: $dat[2] --> atomname\n";
      print "dat_3: $dat[3] --> resname\n";
      print "dat_4: $dat[4] --> segname\n";
      print "dat_5: $dat[5] --> junk\n";
      print "dat_6: $dat[6] --> x\n";
      print "dat_7: $dat[7] --> y\n";
      print "dat_8: $dat[8] --> z\n";
...
###      if($hat2 > 1 and ($segnam[$hat] ne $segnam[$hat-1])) {$multi_seg++;$tem=$nca[$np]}
      ...
      if($atomnam[$hat] eq 'N' or $atomnam[$hat] eq 'CA' or $atomnam[$hat] eq 'C' or $atomnam[$hat] eq 'O' or $atomnam[$hat] eq 'OT1' or $atomnam[$hat] eq 'OT2') #or ($atomnam[$hat] eq 'CD' and $resnam[$np][$hat] eq "PRO"))
        {
        $idbb[$hat2] = 1;
        if($attype[$hat2-1] eq 's') 
          {
          $send[$resnum[$hat2-1]] = $hat2-1;
          }
        $attype[$hat2] = "b";
        $nb[$resnum[$hat2]]++;
        if($atomnam[$hat] eq 'CA') 
          ...
          $yb2[$nca2] = $y[$hat2];
          $zb2[$nca2] = $z[$hat2];
          $seg[$np][$nca[$np]] = $segid[$hat2];
          print "\n\n$resnum[$hat2], $resnam[$hat2]\n";
          if($resnam[$hat2] eq 'GLY') 
            {
            $ngly++;
            $nsc++;
        ...
        if($ns[$resnum[$hat2]] == 1) 
          ...
          $sstrt[$resnum[$hat2]] = $hat2;
          }
        }
      }
    }  
  close IN;
  for($i=1;$i<=$nca[$np];$i++) {$i2=$i+$nca2-$nca[$np];print "$i2: $gly[$i2], $ns[$i2]\n"}
  if($attype[$hat2] eq 's')
    {
    $send[$resnum[$hat2]] = $hat2;
    }
...
#  else {$maxres[$np] = $resnum[$hat]}
  ...
  print "maxres = $nca2, maxatoms = $maxat\n";
  ...
  for($i=1;$i<=$maxres[$np];$i++)
    {
    $j=$i+$nca2-$nca[$np];
    if($nb[$j] != 4 and $i != $maxres[$np]) 
      {
      die "ERROR: In pdb $pdb[$np] (# $np) the number of backbone atoms in residue $i incorrect: $nb[$j] != 4\n";
      }
    elsif(($nb[$j] != 4 and $nb[$j] != 5) and $i == $maxres[$np])
      {
      die "ERROR: In pdb $pdb[$np] the number of backbone atoms in residue $i incorrect: $nb[$j] != 4\n";
      }
    }
  ...
  for($i=1;$i<=$maxres[$np];$i++)
    {
    $j=$i+$nca2-$nca[$np];
    if($refNscat{uc($resnam_num[$j])} != $ns[$j]) {die "ERROR 1: number of heavy atoms in sidechain $i ($resnam_num[$j]) incorrect: $refNscat{uc($resnam_num[$j])} != $ns[$j]\n"}
    }
  ...
  for($i=1;$i<=$nca[$np];$i++)
    ...
    if(uc($pot[$np]) !~ m/GENERIC/)
      { 
      $resnam[$np][$i] = @alphabet[$np+12] . "$i";
      $newnam[$np][$i][1] = "$cap[$np]" . "$i";
#      $newnam[$np][$i][1] = "$cap[$np]" . "$cnt";
#      $newnam[$np][$i][2] = "$scp[$np]" . "$cnt";
      }
...
if(uc($pot[0]) !~ m/GENERIC/)
  ...
  for($i=1;$i<=$nca2;$i++)
    {
    $ncsc[$i]=0;
    for($j=1;$j<=$nca2;$j++) {$distances[$i][$j] = distance($xb2[$i],$yb2[$i],$zb2[$i],$xb2[$j],$yb2[$j],$zb2[$j])}
    }
  print "Finished calculating distance matrix\n";
  $nnatsc=0;
  $nonnatsc=0;
  $nbsc=0;
#Part Ia (contacts inside each chain)
  $li=1;
  $ri=0;
  $lj2=1;
  $rj2=0; 
  for($np=0;$np<=$#pdb;$np++) #
    ...
    $ri=$ri+$nca[$np]; 
    $rj2=$rj2+$hat_np[$np];
    for($i=$li;$i<=$ri-3;$i++) # sidechain 1
      {
      for($j=$i+3;$j<=$ri;$j++) # sidechain 2
        {
#        $dij = distance($xb2[$i2],$yb2[$i2],$zb[$i2],$xb2[$j],$yb2[$j],$zb2[$j]);
#        $dij = $distances[$i][$j];
          ...
          for($k=$sstrt[$i];$k<=$send[$i];$k++)
            ...
            for($l=$sstrt[$j];$l<=$send[$j];$l++)
              ...
              if($dij <= $heav_cut and $found == 0)
                ...
                else {$dij = $distances[$i][$j]} 
		#distance($xb[$i],$yb[$i],$zb[$i],$xb[$j],$yb[$j],$zb[$j])}
                $natdist[$nnatsc] = $dij;
                $native[$i][$j]=1;
                }
              }
...
#    $nbsc=0;
    ...
    for($i=$li;$i<=$ri;$i++) # loop through Ca only
      {
      for($j=$lj2;$j<=$rj2;$j++) # loop through all heavy atoms     
        {
          ...
          if($gly[$i] == 0 and $idbb[$j] == 1 and $found[$resid][$i] != 1) # is it a backbone atom?
            ...
            for($k=$sstrt[$i];$k<=$send[$i];$k++) # loop through sidechain $i's heavy atoms
              ...
              if($dij <= $heav_cut and $found[$resid][$i] != 1)
                ...
                else {$dij = $distances[$i][$resid]} 
	#distance($xb[$i],$yb[$i],$zb[$i],$xb[$resid],$yb[$resid],$zb[$resid])}
                $nbsc++;
                $natbsc[$nbsc][1] = $resid; # backbone ID
                $natbsc[$nbsc][2] = $i; # sidechain ID
                $natdist_bsc[$nbsc] = $dij;
                $native[$i][$resid]=1;
                }      
              }
            }
    ...
    print "# nat sc-sc contacts $nnatsc, # nat sc-bb contacts $nbsc, and  # non-nat sc-sc $nonnatsc\n";
    $li=$li+$nca[$np];
    $lj2=$lj2+$hat_np[$np];
    }
    &visualwritepdb;
    print "$fileBaseName\n";
    open(CM,">NumContacts_perSC.dat");
    for($i=1;$i<=$nca2;$i++)
      {
      print CM "$i $ncsc[$i]\n";
      }
    close CM;
#Part Ib (contacts between chains if having more than one chain)  
  if($#pdb>0)
   {
   $lii[0]=1;
   $rii[0]=$nca[0];
   $ljj2[0]=1;
   $rjj2[0]=$hat_np[0];
   for($np=1;$np<=$#pdb;$np++) #define range of (li,ri) for each chain 
     {
     $lii[$np]=$lii[$np-1]+$nca[$np-1];
     $rii[$np]=$rii[$np-1]+$nca[$np];     
     $ljj2[$np]=$ljj2[$np-1]+$hat_np[$np-1];
     $rjj2[$np]=$rjj2[$np-1]+$hat_np[$np];
     }
   for($np1=0;$np1<=$#pdb-1;$np1++)
     {
     for($np2=$np1+1;$np2<=$#pdb;$np2++)
       { 
       for($i=$lii[$np1];$i<=$rii[$np1];$i++) # sidechain 1
         {
         for($j=$lii[$np2];$j<=$rii[$np2];$j++) # sidechain 2
           {
#        $dij = distance($xb2[$i2],$yb2[$i2],$zb[$i2],$xb2[$j],$yb2[$j],$zb2[$j]);
#           $dij = $distances[$i][$j];
           if($gly[$i] == 0 and $gly[$j] == 0)
    ...
    # (4) Compute native contacts between backbone and side-chains
#    $nbsc=0;
       print "Determining Backbone - Side chain contacts\n";#contacts between sc1-bb2
       for($i=$lii[$np1];$i<=$rii[$np1];$i++) # sidechain 1
         {
         for($j=$ljj2[$np2];$j<=$rjj2[$np2];$j++) # loop through all heavy atoms     
           {
           $resid = $resnum[$j]; # id residue of heavy atom
#          if($i < $resid - 2 or $i > $resid + 2)
#           {
...
#           }
       ...
       print "Determining Backbone - Side chain contacts\n";#contacts between bb1-sc2
       for($i=$lii[$np2];$i<=$rii[$np2];$i++) # sidechain 1
         {
         for($j=$ljj2[$np1];$j<=$rjj2[$np1];$j++) # loop through all heavy atoms     
           {
           $resid = $resnum[$j]; # id residue of heavy atom
#          if($i < $resid - 2 or $i > $resid + 2)
#           {
  ...
  #    {
      ...
      foreach $line (@D)
        ...
        if( $line =~ m/ACC/ or $line =~ m/DNR/ )
          ...
          $RESnum1 =~ s/\s+$//;
          $RESnum2 =~ s/\s+$//;
#          $segment1=$segnam2[$RESnum1];
#          $segment2=$segnam2[$RESnum2]; 
          $segment1 =~ s/\s+$//;
          $segment2 =~ s/\s+$//;
...
#          print"$nRes1,$nRes2\n";
          $native[$nRes1][$nRes2]=1;
          $native[$nRes2][$nRes1]=1;
          if($nRes1 > $nca2 or $nRes2 > $nca2) {die "ERROR: here\n"}
          if($segment1 eq $segment2)
            ...
            if($nRes1 < $nRes2) 
              ...
              if(defined($hbond[$nRes1][$nRes2])) 
                {
           #     print "$nRes1,$nRes2,$segment1,$segment2\n";
                if($helical[$nRes1] == 1 and $helical[$nRes2] == 1 ) {$hbond_ene[$nhbond] = 2*$single_hbond_ene_helix} # helical hbond
                else {$hbond_ene[$nhbond] = 2*$single_hbond_ene} # not helical hbond
                }
              else 
                {
                $nhbond++;
                $hbond[$nRes1][$nRes2] = 1;
                $hbond_at[$nhbond][1] = $nRes1;
                $hbond_at[$nhbond][2] = $nRes2;
                if($helical[$nRes1] == 1 and $helical[$nRes2] == 1 ) {$hbond_ene[$nhbond] = $single_hbond_ene_helix} # helical hbond
                else {$hbond_ene[$nhbond] = $single_hbond_ene} # not helical hbond
                }
              }
            }
          else
      ...
      for($i=1;$i<=$nhbond;$i++) {print "$i $hbond_ene[$i], $hbond_at[$i][1] $hbond_at[$i][2]\n"}
      print "# of unique Hbonds $nhbond\n";
#      }
    for($i=1;$i<=$nhbond;$i++)
      {
      $n1 = $hbond_at[$i][1];
      $n2 = $hbond_at[$i][2];
      $dhb[$i] = distance($xb2[$n1],$yb2[$n1],$zb2[$n1],$xb2[$n2],$yb2[$n2],$zb2[$n2]);
      print "$hbond_at[$i][1],$hbond_at[$i][2] $dhb[$i]\n";
      }
  }
#End Part II
...
for($np=0;$np<=$#pdb;$np++)
  ...
  for($i=1;$i<=$nca[$np];$i++)
    ...
    if($casm==1)
      ...
      {
      printf OUT "MASS %-5s%-9s%-10.6f\n",$cnt,$newnam[$np][$i][1],$aaSCmass{$resnam_num1[$np][$i]}; 
#      printf OUT "MASS %-5s%-9s%-10.6f\n",$i,$newnam[$np][$i][1],$aaSCmass{$resnam_num1[$np][$i]};  
     }
    }
  }
print OUT "\n";
...
for($np=0;$np<=$#pdb;$np++)
  ...
  for($i=1;$i<=$nca[$np];$i++)
    ...
    if($casm==1)
      ...
      {
      printf OUT "MASS %-5s%-9s%-10.6f\n",$cnt,$newnam[$np][$i][1],$aaSCmass{$resnam_num[$cnt]};
      }
    }
  }
    ...
    for($i=1;$i<=$maxres[$np]-3;$i++)
     {
      if($seg[$np][$i] == $seg[$np][$i+1] and $seg[$np][$i] == $seg[$np][$i+2] and $seg[$np][$i] == $seg[$np][$i+3])
        ...
        for($j=1;$j<=4;$j++) 
          {
          $kd=$dihedb_nongo[$res{$resnam_num1[$np][$i+1]}][$res{$resnam_num1[$np][$i+2]}][$j][1];
          $period=$dihedb_nongo[$res{$resnam_num1[$np][$i+1]}][$res{$resnam_num1[$np][$i+2]}][$j][2];
          $delta=$dihedb_nongo[$res{$resnam_num1[$np][$i+1]}][$res{$resnam_num1[$np][$i+2]}][$j][3];
      ...
      for($i=$li;$i<=$ri;$i++)
        ...
        for($j=$li;$j<=$ri;$j++)
         ...
         if(abs($i-$j)>=3)
          ...
          if($native[$i][$j] != 1)
            {
            if($distances[$i][$j] < $sigmin[$i]) {$sigmin[$i]=$distances[$i][$j]}
            }
          }
         }
        # Check that the collision diameter is not outrageously large 
        if($sigmin[$i] > 3*$rvdw{$resnam_num[$i]})
          {
          print "Adjusting collision diameter for residue $i ($resnam_num[$i]) from $sigmin[$i] to 3*$rvdw{$resnam_num[$i]}\n";
          $sigmin[$i]=3*$rvdw{$resnam_num[$i]};
          }
        }  
        $li=$li+$nca[$np];
      }
...
if($#pdb>0)
  ...
  for($np1=0;$np1<=$#pdb;$np1++)
    {
    for($np2=0;$np2<=$#pdb;$np2++)
      {
      if ($np1!=$np2)
       {
       for($i=$lii[$np1];$i<=$rii[$np1];$i++) # sidechain 1
        ...
        for($j=$lii[$np2];$j<=$rii[$np2];$j++) # sidechain 2
          {
          if($sigmin[$i]>$distances[$i][$j]){$sigmin[$i]=$distances[$i][$j]} 
          }
        }
       } 
      }
...
#here for Ca only model
      {
      $eps2 = -0.000132;
      $rmin2 = $sigmin[$cnt]*2**(1/6)/2;
      $temp = $fnn*$rmin2;
...
#This section for Ca only model
  else
    {
    if(uc($pot[0]) !~ m/GENERIC/) # C-alpha model
      {
      print OUT "! b-b due to Hbonding plus native side-chain interactions plus backbone-sidechain interactions, from PDB $compl\n";
      # Add up non-bonded energies
      @energy=();
      # loop through hydrogen bonds
      for($i=1;$i<=$nhbond;$i++)
        {
        $distance[$hbond_at[$i][1]][$hbond_at[$i][2]] = $dhb[$i];
#        if($ndomain[$np] == 0) # If interface not defined
#          {
          $energy[$hbond_at[$i][1]][$hbond_at[$i][2]]+=$hbond_ene[$i];
          $energy[$hbond_at[$i][2]][$hbond_at[$i][1]]+=$hbond_ene[$i];
#          }
        }
      # loop through sc-sc interactions
      for($i=1;$i<=$nnatsc;$i++)
        {
        $distance[$nsc1[$i]][$nsc2[$i]] = $natdist[$i];
        if($eps[$res{$resnam_num[$nsc1[$i]]}][$res{$resnam_num[$nsc2[$i]]}] == 0)
          {
          print "$i, $nsc1[$i]-$nsc2[$i], $resnam_num[$nsc1[$i]]-$resnam_num[$nsc2[$i]],$res{$resnam_num[$nsc1[$i]]}-$res{$resnam_num[$nsc2[$i]]}\n";
          die "ERROR 2: Well depth equal to zero!!! $eps[$res{$resnam_num[$nsc1[$i]]}][$res{$resnam_num[$nsc2[$i]]}]\n";
          }
#        if($ndomain[$np] == 0) # If interface not defined
#          {
          $energy[$nsc1[$i]][$nsc2[$i]]+=$eps[$res{$resnam_num[$nsc1[$i]]}][$res{$resnam_num[$nsc2[$i]]}];
          $energy[$nsc2[$i]][$nsc1[$i]]+=$eps[$res{$resnam_num[$nsc1[$i]]}][$res{$resnam_num[$nsc2[$i]]}];
#          }
        }
      # loop through bb-sc interactions    for($i=1;$i<=$nbsc[$np];$i++)
      for($i=1;$i<=$nbsc;$i++)
        {
        $distance[$natbsc[$i][1]][$natbsc[$i][2]]=$natdist_bsc[$i];
        $distance[$natbsc[$i][2]][$natbsc[$i][1]]=$natdist_bsc[$i];
#        if($ndomain[$np] == 0) # If interface not defined
#          {
          $energy[$natbsc[$i][1]][$natbsc[$i][2]]+=$ene_bsc;
          $energy[$natbsc[$i][2]][$natbsc[$i][1]]+=$ene_bsc;
#          }
        }
      # Print out NBFIX energy values
  #loop1 through each chain
       $li=1;
       $ri=0;
       $i1=0;
       for($np=0;$np<=$#pdb;$np++)
         ...
         for($i=$li;$i<=$ri-2;$i++)
           ...
           for($j=$i+2;$j<=$ri;$j++)
             {
             if($energy[$i][$j]>0) {printf OUT "%-8s%-11s%-11.6f%-11.6f\n",$newnam[$np][$i-$i1][1],$newnam[$np][$j-$i1][1],-$energy[$i][$j],$distance[$i][$j]}
             }
           }
         $li=$li+$nca[$np];
         $i1=$li-1;
         }
   #loop2 through non-bond inter chains        
       if($#pdb>0)
        {
        $lii[0]=1;
        $rii[0]=$nca[0];
          ...
          for($np2=$np1+1;$np2<=$#pdb;$np2++)
            ...
            for($i=$lii[$np1];$i<=$rii[$np1];$i++) # sidechain 1
              ...
              for($j=$lii[$np2];$j<=$rii[$np2];$j++) # sidechain 2
                ...
                if($energy[$i][$j]>0) {printf OUT "%-8s%-11s%-11.6f%-11.6f\n",$newnam[$np1][$i-$i1][1],$newnam[$np2][$j-$j1][1],-$energy[$i][$j],$distance[$i][$j]}
                }
              } 
            }
            $i1=$i1+$nca[$np1];
...
#End of section "Ca-only model" and "(uc($pot[0]) !~ m/GENERIC/)"
    else
      {
      print OUT "!Generic interactions between unstructured portions of this protein, from PDB $pdb[$np]\n";
      # Print out NBFIX energy values
...
sub writecrd
  ...
  for($np=0;$np<=$#pdb;$np++)
    ...
    for($i=1;$i<=$nca[$np];$i++)
      {
      $count++;
      $X = sprintf("%7.3f",$xb2[$count]);
      $Y = sprintf("%7.3f",$yb2[$count]);
      $Z = sprintf("%7.3f",$zb2[$count]);
      $Xf = sprintf("%7.7s",$X);
      $Yf = sprintf("%7.7s",$Y);
      $Zf = sprintf("%7.7s",$Z);
#      printf OUT "%5s%5s %-4s %-3s%11.5f%10.5f%10.5f%2s%5s%13.5f\n",$count,$count,$resnam[$np][$i],A,$Xf,$Yf,$Zf,$seg_name,1,0;
      printf OUT "%5s%5s %-4s %-3s%11.5f%10.5f%10.5f%2s%5s%13.5f\n",$count,$count,@alphabet[$np+12] . "$i",A,$Xf,$Yf,$Zf,@alphabet[$seg[$np][$i]-1],$i,0;
#      if($seg[$np][$i]!=$seg[$np][$i+1]){$count=0};
      printf OUT2 " $resnam_num[$count]";
      if($ns[$np][$i] > 0 and $casm==1)
        {
...
GENERATE @alphabet[$ind] FIRS none LAST none SETUP

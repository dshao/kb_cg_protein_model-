...
#   (2020/03/10)In legacy code the collision diameter was calculated in the following way:
#   Assume a 100-residue protein. i can will take values {i = 1, 2, ..., 96, 97}. 
#   When i = 1, j can take values {j = 4, 5, ... , 99, 100},
#   which correctly checks all residues that may be closest to i = 1 without sharing a native contact.
#   However, when i = 50, j will take values {j = 53, 54, ..., 99, 100}.
#   This means that the possibility that i is closest to one of the residues {j = 1, 2, ..., 46, 47}
#   is not considered. This may result in larger than necessary repulsive radii.
#   Verion 36.1 and beyond, repulsive radii of some middle residues (among 4 to Natom_of_chain - 3)
#   can reduce, and the non-bonded interaction can changes alot as Lan saw from his simulations 
#   (you can check this by making a comparision between the old and new pamameter files of your simulations)
#
=pod
...
@dat = split(/\//,$dir);
$root = join('/',@dat[0..4]);
if(uc($final_pot) =~ m/MJ/) {$miya = "/gpfs/group/epo2/default/for_ian/software/shared_files/mj_contact_potential.dat"}
elsif(uc($final_pot) =~ m/KGS/) {$miya = "/gpfs/group/epo2/default/for_ian/software/shared_files/kgs_contact_potential.dat"}
elsif(uc($final_pot) =~ m/BT/) {$miya = "/gpfs/group/epo2/default/for_ian/software/shared_files/bt_contact_potential.dat"}
elsif(uc($final_pot) =~ m/CT/) {$miya = "/gpfs/group/epo2/default/for_ian/software/shared_files/mj_contact_potential.dat"} # Dirty, temporary solution
elsif(uc($final_pot) =~ m/GENERIC/) {$miya = "/gpfs/group/epo2/default/for_ian/software/shared_files/mj_contact_potential.dat"} # Dirty, temporary solution
...
if($dihedral_go[$np] == 0) 
 {
 $file = "/gpfs/group/epo2/default/for_ian/software/shared_files/karanicolas_dihe_parm.dat";
...
for($np=0;$np<=$#pdb;$np++)
  ...
  system "/gpfs/group/epo2/default/for_ian/software/mmtsb2.0/perl/convpdb.pl -nsel heavy $pdb[$np] > temp.pdb";
  system "/gpfs/group/epo2/default/for_ian/software/mmtsb2.0/perl/convpdb.pl -renumber 1 -nohetero -out charmm27 temp.pdb > $out";
...
print OUT "CUTNB 23 CTOFNB 20 CTONNB 18 EPS 78.5 WMIN 1.5 E14FAC 1.0\n";
    ...
    if(uc($pot[$np]) =~ m/GENERIC/)
      ...
      for($i=$li;$i<=$ri;$i++)
        ...
        for($j=$li;$j<=$ri;$j++)
         ...
         if(abs($i-$j)>=3)
          {
          #print "\nChecking if res $i & $j are NOT in contact\n";
          if($native[$np][$i][$j] != 1)
            {
            #print "-->Res $i & $j are NOT in contact\n";
            #print "Checking if dist($i,$j):$distances[$np][$i][$j] < sigmin_$i:$sigmin[$np][$i]\n";
            if($distances[$np][$i][$j] < $sigmin[$np][$i]) 
              {
              #print "--> dist($i,$j) is < sigmin_$i:$sigmin[$np][$i]\n";
              #print "--> setting sigmin_$i from $sigmin[$np][$i] to $distances[$np][$i][$j]\n";
              $sigmin[$np][$i]=$distances[$np][$i][$j]
              }
            }
          }
         }
...
sub Stride
  ...
  if( -e $out )
    ...
    if( -s $out  )
      {
      $cmd = "/gpfs/group/epo2/default/for_ian/software/stride/stride $out -h |"; # calc main-chain H-bonds
